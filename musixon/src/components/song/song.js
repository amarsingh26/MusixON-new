...
const SongsComponent = () => {
    ...
    return (
                ...
                <div className="w-full p-4 flex items-center justify-center flex-col" class="first">
                    <div class='firstsecond'>
                        <input type="text" placeholder="search here ..." value={songFilter} onChange={(e) => setSongFiter(e.target.value)} />
                    </div>

                    {/* main container */}
                    <div class='second' className="border my-4 p-4 w-full border-blue-400 rounded-400">
...
export const SongCard = ({ data, index }) => {
    ...
    return (
        ...
        >
            ...
            <p className="text-base text-headingColor font-semibold my-2 songcard">
                {data.name.length > 25 ? `${data.name.slice(0, 25)}` : data.name}
                <div className="block text-sm text-gray-400 my-1">{data.artist}</div>
            </p>
            </div>
